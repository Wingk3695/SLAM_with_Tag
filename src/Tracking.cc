    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    if(settings){
        ...
        if(sensor==System::IMU_MONOCULAR || sensor==System::IMU_STEREO || sensor==System::IMU_RGBD)//删除
        {
...
Sophus::SE3f Tracking::GrabImageMulti(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const cv::Mat &imRectSideLeft, const cv::Mat &imRectSideRight, const double &timestamp, string filename)
{
    ...
    //std::cout << "当前位姿：" << mCurrentFrame.GetPose().inverse().matrix() << endl;
    return mCurrentFrame.GetPose().inverse();
}
...
{   
    ...
    //           << "GrabImuData: t=" << imuMeasurement.t
    //           << " a=[" << imuMeasurement.a.transpose() << "]"
    //           << " w=[" << imuMeasurement.w.transpose() << "]"
    //           << std::endl; 
    //
...
void Tracking::PreintegrateIMU()
{
    ...
    if(mlQueueImuData.size() == 0)
    {
        ...
        //调试
        // std::cout << "Not IMU data in mlQueueImuData!! Size = " << mlQueueImuData.size() << std::endl;
        //
        mCurrentFrame.setIntegrated();
        return;
    }
    ...
    while(true)
    {
        bool bSleep = false;
        {
            ...
            if(!mlQueueImuData.empty())
            {
                ...
                cout.precision(17);
                //调试
                // std::cout << "IMU timestamp: " << m->t 
                //             << " Current Frame Timestamp: " << mCurrentFrame.mTimeStamp
                //             << " mImuPer: " << mImuPer << std::endl;
                //
                ...
                {
                    //调试
                    // std::cout << "Pushing IMU data to mvImuFromLastFrame: " 
                    //             << " t=" << m->t
                    //             << " a=[" << m->a.transpose() << "]"
                    //             << " w=[" << m->w.transpose() << "]" << std::endl;
                    //
