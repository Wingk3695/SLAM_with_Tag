...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("ORB-SLAM3-Multi without IMU: Map Viewer",1024,768);
    ...
    Ow.SetIdentity();
    cv::namedWindow("ORB-SLAM3-Multi without IMU: Current Frame");
    ...
    while(1)
    {
        ...
        cv::imshow("ORB-SLAM3-Multi without IMU: Current Frame",toShow);
        cv::waitKey(mT);

        if(menuReset)
        {
            menuShowGraph = true;
            menuShowInertialGraph = false;
